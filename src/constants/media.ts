...
export const MIME_TO_CATEGORY: Record<string, AllowedMimeCategory> = {
	...Object.fromEntries(ALLOWED_IMAGE_MIMES.map((m) => [m, "image" as const])),
	...Object.fromEntries(
		ALLOWED_DOCUMENT_MIMES.map((m) => [m, "document" as const]),
	),
	...Object.fromEntries(ALLOWED_VIDEO_MIMES.map((m) => [m, "video" as const])),
	...Object.fromEntries(ALLOWED_AUDIO_MIMES.map((m) => [m, "audio" as const])),
};
