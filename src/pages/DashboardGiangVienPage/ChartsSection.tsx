...
export function ChartsSection({ weeklyData, topicScores }: ChartsSectionProps) {
  return (
    <div className="gv-charts-row">
      <ChartCard title="Giờ Dạy & Học Viên Theo Tuần" variant="dark">
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={weeklyData} margin={{ top: 4, right: 16, left: -10, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey="gioDay" name="Giờ dạy" stroke="#3b82f6" strokeWidth={2} dot={{ r: 4, fill: '#3b82f6' }} />
            <Line type="monotone" dataKey="hocVien" name="Học viên" stroke="#fdb913" strokeWidth={2} dot={{ r: 4, fill: '#fdb913' }} />
          </LineChart>
        </ResponsiveContainer>
      </ChartCard>

      <ChartCard title="Điểm Trung Bình Các Chủ Đề" variant="dark">
        <ResponsiveContainer width="100%" height={260}>
          <RadarChart data={topicScores} cx="50%" cy="50%" outerRadius={90} margin={{ top: 8, right: 24, bottom: 8, left: 24 }}>
            ...
            <Radar name="Điểm TB" dataKey="score" stroke="#fdb913" fill="#fdb913" fillOpacity={0.35} dot={{ r: 4, fill: '#fdb913' } as object} />
            <Tooltip {...DARK_TOOLTIP} />
          </RadarChart>
        </ResponsiveContainer>
      </ChartCard>
